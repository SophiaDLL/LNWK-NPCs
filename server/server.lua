...
local function loadNPCData()
    local jsonData = LoadResourceFile(GetCurrentResourceName(), "shared/npcs.json") or "[]"
    return json.decode(jsonData)
end

local function saveNPCData(data)
    SaveResourceFile(GetCurrentResourceName(), "shared/npcs.json", json.encode(data, { indent = true }), -1)
end
...
AddEventHandler("falcon:previewAnimation", function(data)
    local src = source
    TriggerClientEvent("falcon:previewAnimation", src, data)
end)
...
RegisterCommand('pedui', function(source, args)
    local src = source
    -- local Player = QBCore.Functions.HasPermission(src, "admin") or QBCore.Functions.HasPermission(src, "god")
    if not Player then return end
